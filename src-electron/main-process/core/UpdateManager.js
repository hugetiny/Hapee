...
export default class UpdateManager extends EventEmitter {
  ...
  updateAvailable (event, info) {
    ...
    dialog.showMessageBox({
      type: 'info',
      title: this.i18n.t('check-for-updates-title'),
      message: this.i18n.t('update-available-message'),
      buttons: [this.i18n.t('yes'), this.i18n.t('no')],
      cancelId: 1
    }, (buttonIndex) => {
      if (buttonIndex === 0) {
  ...
  updateNotAvailable (event, info) {
    ...
    if (this.autoCheckData.userCheck) {
      dialog.showMessageBox({
        title: this.i18n.t('check-for-updates-title'),
        message: this.i18n.t('update-not-available-message')
      })
    }
  }
  ...
  updateDownloaded (event, info) {
    ...
    dialog.showMessageBox({
      title: this.i18n.t('check-for-updates-title'),
      message: this.i18n.t('update-downloaded-message')
    }, () => {
  ...
  updateError (event, error) {
    ...
    this.updater.logger.warn(`[negibox] update-error: ${msg}`)
