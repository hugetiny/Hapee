...
export default class Application extends EventEmitter {
  ...
  startEngine () {
    try {
      ...
      dialog.showMessageBox({
        type: 'error',
        title: this.i18n.t('system-error-title'),
        message: this.i18n.t('system-error-message', { message })
      }, () => {
        setTimeout(() => {
          app.quit()
        }, 100)
  ...
  handleFile (filePath) {
    ...
    readFile(filePath, (err, data) => {
      if (err) {
        logger.warn(`[negibox] read file error: ${filePath}`, err.message)
        return
      }
  ...
  handleCommands () {
    ...
    this.on('application:change-locale', (locale) => {
      logger.info('[negibox] application:change-locale===>', locale)
  ...
  handleIpcMessages () {
    ...
    // FROM NEGIBOX
    ipcMain.on('update-progress', (event, arg) => {
      if (arg && arg !== 1) {
        WindowManager.setProgressBar(arg)
      }
    })
