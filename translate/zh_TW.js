...
translations.zh_TW = {
  ...
  "RPC Settings": "RPC 設定",
  "Negibox Debug Console": "Negibox 偵錯控制台",
  "Engine Status": "引擎狀態",
  "File Name": "檔名",
  "File Size": "大小",
  "Progress": "進度",
  ...
  "Confirm Retry": "確認重試",
  "Are you sure you want to retry the selected task? Negibox will create same task after clicking OK.": "您是否要重試選中的工作? 點選 確定 後, Negibox 將會建立相同的工作.",
  ...
  "Engine RPC Alias": "引擎 RPC 別名",
  "Engine RPC Address": "引擎 RPC 位址",
  "Engine RPC Protocol": "引擎 RPC 協定",
  "Engine RPC Http Request Method": "引擎 RPC Http 要求方法",
  "POST method only supports aria2 v1.15.2 and above.": "POST 方法僅支援 aria2 v1.15.2 及以上.",
  "Engine RPC Secret Token": "引擎 RPC 金鑰",
  "Activate": "啟用",
  "Reset Settings": "重設設定",
  "Confirm Reset": "確認重設",
  ...
  "Recently Used": "最近使用",
  "RPC Alias": "RPC 別名",
  "Import / Export Settings": "匯入/匯出  設定",
  ...
  "Negibox Title": "Negibox 標題",
  "Downloading Count": "正在下載數量",
  "Waiting Count": "稍后下载數量",
  "Stopped Count": "已停止數量",
  ...
  "Http and WebSocket would be disabled when accessing Negibox via Https.": "使用 Https 訪問 Negibox 時，Http 和 WebSocket 將被停用.",
  "POST": "POST",
  "GET": "GET",
  "Disabled": "停用",
  ...
  "Tap to configure and get started with Negibox.": "您還沒有進行過設定, 點選這裡進行設定.",
  "Cannot initialize WebSocket!": "無法初始化 WebSocket!",
  "Access Denied!": "拒絕訪問!",
  "You cannot use Negibox because this browser does not support data storage.": "您無法使用 Negibox, 您的瀏覽器不支援資料儲存.",
  "task detail": "詳細資訊",
  "error": {
    "unknown": "不詳錯誤.",
  ...
  "options": {
    ...
    "all-proxy.description": "設定所有協定的代理伺服器位址. 如果覆蓋之前設定的代理伺服器, 使用  即可. 您還可以針對特定的協定覆蓋此選項, 即使用 --http-proxy, --https-proxy 和 --ftp-proxy 選項. 此設定將會影響所有下載. 代理伺服器位址的格式為 [http://][USER:PASSWORD@]HOST[:PORT].",
    ...
    "dry-run.name": "模擬運行",
    "dry-run.description": "如果設定為是, aria2 將僅檢查遠端檔案是否存在而不會下載檔案內容. 此選項僅對 HTTP/FTP 下載生效. 如果設定為 true, BT 下載將會直接取消.",
    ...
    "stream-piece-selector.description": "指定 HTTP/FTP 下載使用的分段選擇演算法. 分段表示的是並行下載時固定長度的分隔段. 如果設定為預設, aria2 將會依減少建立連線數選擇分段. 由於建立連線操作的成本較高, 因此這是合理的預設行為. 如果設定為順序, aria2 將選擇索引最小的分段. 索引為 0 時表示為檔案的第一個分段. 這將有助於視頻的邊下邊播. --enable-http-pipelining 選項有助於減少重連線的開銷. 請注意, aria2 依賴於 --min-split-size 選項, 所以有必要對 --min-split-size 選項設定一個合理的值. 如果設定為隨機, aria2 將隨機選擇一個分段. 就像順序一樣, 依賴於 --min-split-size 選項. 如果設定為幾何, aria2 會先選擇索引最小的分段, 然後會為之前選擇的分段保留指數增長的空間. 這將減少建立連線的次數, 同時檔案開始部分將會先行下載. 這也有助於視頻的邊下邊播.",
    "timeout.name": "超時時間",
    "timeout.description": "",
    "uri-selector.name": "URI 選擇演算法",
    "uri-selector.description": "指定 URI 選擇的演算法. 可選的值包括 依順序, 反饋 和 自適應. 如果設定為依順序, URI 將依清單中出現的順序使用. 如果設定為反饋, aria2 將根據之前的下載速度選擇 URI 清單中下載速度最快的伺服器. 同時也將有效跳過無效鏡像. 之前統計的下載速度將作為伺服器狀態檔案的一部分, 參見 --server-stat-of 和 --server-stat-if 選項. 如果設定為自適應, 將從最好的鏡像和保留的連線裡選擇一項. 補充說明, 其返回的鏡像沒有被測試過, 同時如果每個鏡像都已經被測試過時, 返回的鏡像還會被重新測試. 否則, 其將不會選擇其他鏡像. 例如反饋, 其使用伺服器狀態檔案.",
    ...
    "http-auth-challenge.description": "僅當伺服器需要時才發送 HTTP 認證要求標頭. 如果設定為否, 每次都會發送認證要求標頭. 例外: 如果使用者名稱和密碼包含在 URI 中, 將忽略此選項並且每次都會發送認證要求標頭.",
    "http-no-cache.name": "停用快取",
    "http-no-cache.description": "發送的要求標頭中將包含 Cache-Control: no-cache 和 Pragma: no-cache header 以避免內容被快取. 如果設定為否, 上述要求標頭將不會發送, 同時您也可以使用 --header 選項將 Cache-Control 要求標頭加入進去.",
    ...
    "ftp-pasv.name": "被動模式",
    "ftp-pasv.description": "在 FTP 中使用被動模式. 如果設定為否, 則使用主動模式. 此選項不適用於 SFTP 傳輸.",
    ...
    "ssh-host-key-md.description": "設定 SSH 主機公鑰的總和檢查碼. TYPE 為雜湊類型. 支援的雜湊類型為 sha-1 和 md5. DIGEST 是十六進位摘要. 例如: sha-1=b030503d4de4539dc7885e6f0f5e256704edf4c3. 此選項可以在使用 SFTP 時用來驗證伺服器的公鑰. 如果此選項不設定, 即保留預設, 不會進行任何驗證。",
    ...
    "bt-enable-hook-after-hash-check.description": "允許 BT 下載雜湊檢查(參見 -V 選項) 完成後調用命令. 預設情況下, 當雜湊檢查成功後, 通過 --on-bt-download-complete 設定的命令將會被執行. 如果要停用此行為, 請設定為否.",
    ...
    "bt-force-encryption.description": "BT 消息中的內容需要使用 arc4 加密. 此選項是設定 --bt-require-crypto --bt-min-crypto-level=arc4 這兩個選項的快捷方式. 此選項不會修改上述兩個選項的內容. 如果設定為是, 將拒絕以前的 BT 握手, 並僅使用模糊握手及加密消息.",
    "bt-hash-check-seed.name": "做種前檢查檔案雜湊",
    "bt-hash-check-seed.description": "如果設定為是, 當使用 --check-integrity 選項完成雜湊檢查及檔案完成後才繼續做種. 如果您希望僅當檔案損壞或未完成時檢查檔案, 請設定為否. 此選項僅對 BT 下載有效",
    ...
    "bt-require-crypto.description": "如果設定為是, aria 將不會接受以前的 BitTorrent 握手協定(\\19BitTorrent 協定)並建立連線. 因此 aria2 總是模糊握手.",
    ...
    "bt-save-metadata.description": "儲存種子檔案為 .torrent 檔案. 此選項僅對磁力連結生效. 檔名為十六進位編碼後的雜湊值及 .torrent副檔名. 儲存的目錄與下載檔案的目錄相同. 如果相同的檔案已存在, 種子檔案將不會儲存.",
    ...
    "dht-listen-port.description": "設定 DHT (IPv4, IPv6) 和 UDP 伺服器使用的 UCP 埠. 多個埠可以使用逗號 , 分隔, 例如: 6881,6885. 您還可以使用短橫線 - 表示範圍: 6881-6999, 或可以一起使用: 6881-6889, 6999.",
    "dht-message-timeout.name": "DHT 消息超時時間",
    "dht-message-timeout.description": "",
    "enable-dht.name": "啟用 DHT (IPv4",
    "enable-dht.description": "啟用 IPv4 DHT 功能. 此選項同時會啟用 UDP 伺服器支援. 如果種子設定為私有, 即使此選項設定為是, aria2 也不會啟用 DHT.",
    "enable-dht6.name": "啟用 DHT (IPv6",
    "enable-dht6.description": "啟用 IPv6 DHT 功能. 如果種子設定為私有, 即使此選項設定為是, aria2 也不會啟用 DHT. 使用 --dht-listen-port 選項設定監聽的埠.",
    "enable-peer-exchange.name": "啟用節點交換",
    "enable-peer-exchange.description": "啟用節點交換擴充. 如果種子設定為私有, 即使此選項設定為是, aria2 也不會啟用此功能.",
    "follow-torrent.name": "下載種子中的檔案",
    "follow-torrent.description": "如果設定為是或僅記憶體, 當副檔名為 .torrent 或內容類型為 application/x-bittorrent 的檔案下載完成時, aria2 將依種子檔案讀取並下載該檔案中提到的檔案. 如果設定為僅記憶體, 該種子檔案將不會寫入到磁碟中, 而僅會儲存在記憶體中. 如果設定為否, 則 .torrent 檔案會下載到磁碟中, 但不會依種子檔案讀取並且其中的檔案不會進行下載.",
    ...
    "follow-metalink.description": "如果設定為是或僅記憶體, 當副檔名為 .meta4 或 .metalink 或內容類型為 application/metalink4+xml 或 application/metalink+xml 的檔案下載完成時, aria2 將依 Metalink 檔案讀取並下載該檔案中提到的檔案. 如果設定為僅記憶體, 該 Metalink 檔案將不會寫入到磁碟中, 而僅會儲存在記憶體中. 如果設定為否, 則 .metalink 檔案會下載到磁碟中, 但不會依 Metalink 檔案讀取並且其中的檔案不會進行下載.",
    ...
    "metalink-preferred-protocol.description": "指定首選使用的協定. 可以設定為 http, https, ftp 或無. 設定為無時停用此選項.",
    "metalink-enable-unique-protocol.name": "僅使用唯一協定",
    "metalink-enable-unique-protocol.description": "如果一個 Metalink 檔案可用多種協定, 並且此選項設定為是, aria2 將只會使用其中一種. 使用 --metalink-preferred-protocol 參數指定首選的協定.",
    ...
    "rpc-listen-all.description": "在所有網路適配器上監聽 JSON-RPC/XML-RPC 的要求, 如果設定為否, 僅監聽本機網路的要求.",
    ...
    "rpc-save-upload-metadata.description": "在 dir 選項設定的目錄中儲存上傳的種子檔案或 Metalink 檔案. 檔名包括 SHA-1 雜湊後的中繼資料和副檔名兩部分. 對於種子檔案, 副檔名為 '.torrent'. 對於 Metalink 為 '.meta4'. 如果此選項設定為否, 通過 aria2.addTorrent() 或 aria2.addMetalink() 方法加入的下載將無法通過 --save-session 選項儲存.",
    ...
    "allow-piece-length-change.description": "如果設定為否, 當分段長度與控制檔案中的不同時, aria2 將會中止下載. 如果設定為是, 您可以繼續, 但部分下載進度將會丟失.",
    "always-resume.name": "始終斷點續傳",
    "always-resume.description": "始終斷點續傳. 如果設定為是, aria2 始終嘗試斷點續傳, 如果無法恢復, 則中止下載. 如果設定為否, 對於不支援斷點續傳的 URI 或 aria2 遇到 N 個不支援斷點續傳的 URI (N 為 --max-resume-failure-tries 選項設定的值), aria2 會從頭下載檔案. 參見 --max-resume-failure-tries 參數.",
    ...
    "deferred-input.description": "如果設定為是, aria2 在啟動時不會讀取 --input-file 選項設定的檔案中的所有 URI 位址, 而是會在之後需要時依需讀取. 如果輸入檔案中包含大量要下載的 URI, 此選項可以減少記憶體的使用. 如果設定為否, aria2 會在啟動時讀取所有的 URI. 當 -save-session 使用時將會停用 --deferred-input 選項.",
    ...
    "download-result.description": "此選項將修改下載結果的格式. 如果設定為預設, 將列印 GID, 狀態, 平均下載速度和路徑/URI. 如果涉及多個檔案, 僅列印第一個要求檔案的路徑/URI, 其餘的將被忽略. 如果設定為完整, 將列印 GID, 狀態, 平均下載速度, 下載進度和路徑/URI. 其中, 下載進度和路徑/URI 將會每個檔案列印一行. 如果設定為隱藏, 下載結果將會隱藏.",
    "dscp.name": "DSCP",
    ...
    "hash-check-only.description": "如果設定為是, 雜湊檢查完使用 --check-integrity 選項, 根據是否下載完成決定是否終止下載.",
    ...
    "max-resume-failure-tries.description": "當 --always-resume 選項設定為否時, 如果 aria2 檢測到有 N 個 URI 不支援斷點續傳時, 將從頭開始下載檔案. 如果 N 設定為 0, 當所有 URI 都不支援斷點續傳時才會從頭下載檔案. 參見 --always-resume 選項.",
