...
translations.zh_CN = {
  ...
  "RPC Settings": "RPC 设置",
  "Negibox Debug Console": "Negibox 调试控制台",
  "Engine Status": "引擎状态",
  "File Name": "文件名",
  "File Size": "大小",
  "Progress": "进度",
  ...
  "Confirm Retry": "确认重试",
  "Are you sure you want to retry the selected task? Negibox will create same task after clicking OK.": "您是否要重试选中的任务? 点击 确定 后, Negibox 将会创建相同的任务.",
  ...
  "Engine RPC Alias": "引擎 RPC 别名",
  "Engine RPC Address": "引擎 RPC 地址",
  "Engine RPC Protocol": "引擎 RPC 协议",
  "Engine RPC Http Request Method": "引擎 RPC Http 请求方法",
  "POST method only supports aria2 v1.15.2 and above.": "POST 方法仅支持 aria2 v1.15.2 及以上.",
  "Engine RPC Secret Token": "引擎 RPC 密钥",
  "Activate": "激活",
  "Reset Settings": "重置设置",
  "Confirm Reset": "确认重置",
  ...
  "Recently Used": "最近使用",
  "RPC Alias": "RPC 别名",
  "Import / Export Settings": "导入/导出 设置",
  ...
  "Negibox Title": "Negibox 标题",
  "Downloading Count": "正在下载数量",
  "Waiting Count": "稍后下载数量",
  "Stopped Count": "已停止数量",
  ...
  "Functions": "方法",
  "Save Session": "保存会话",
  "Shutdown Engine": "关闭引擎",
  "Confirm Shutdown": "确认关闭",
  "Are you sure you want to shutdown aria2?": "您是否要关闭引擎?",
  "Session has been saved successfully.": "会话已经成功保存.",
  "Engine has been shutdown successfully.": "引擎已经成功关闭.",
  ...
  "Http and WebSocket would be disabled when accessing Negibox via Https.": "使用 Https 访问 Negibox 时，Http 和 WebSocket 将被禁用.",
  "POST": "POST",
  "GET": "GET",
  "Disabled": "禁用",
  ...
  "Tap to configure and get started with Negibox.": "您还没有进行过设置, 点击这里进行设置.",
  "Cannot initialize WebSocket!": "无法初始化 WebSocket!",
  "Access Denied!": "拒绝访问!",
  "You cannot use Negibox because this browser does not support data storage.": "您无法使用 Negibox, 您的浏览器不支持数据存储.",
  "task detail": "详细信息",
  "error": {
    "unknown": "未知错误.",
  ...
  "options": {
    ...
    "stream-piece-selector.description": "指定 HTTP/FTP 下载使用的分片选择算法. 分片表示的是并行下载时固定长度的分隔段. 如果设置为 默认 , aria2 将会按减少建立连接数选择分片. 由于建立连接操作的成本较高, 因此这是合理的默认行为. 如果设置为 顺序, aria2 将选择索引最小的分片. 索引为 0 时表示为文件的第一个分片. 这将有助于视频的边下边播. --enable-http-pipelining 选项有助于减少重连接的开销. 请注意, aria2 依赖于 --min-split-size 选项, 所以有必要对 --min-split-size 选项设置一个合理的值. 如果设置为 随机, aria2 将随机选择一个分片. 就像顺序一样, 依赖于 --min-split-size 选项. 如果设置为几何, aria2 会先选择索引最小的分片, 然后会为之前选择的分片保留指数增长的空间. 这将减少建立连接的次数, 同时文件开始部分将会先行下载. 这也有助于视频的边下边播.",
    "timeout.name": "超时时间",
    "timeout.description": "",
    ...
    "follow-torrent.description": "如果设置为 是 或仅内存, 当后缀为 .torrent 或内容类型为 application/x-bittorrent 的文件下载完成时, aria2 将按种子文件读取并下载该文件中提到的文件. 如果设置为仅内存, 该种子文件将不会写入到磁盘中, 而仅会存储在内存中. 如果设置为 否  , 则 .torrent 文件会下载到磁盘中, 但不会按种子文件读取并且其中的文件不会进行下载.",
    ...
    "follow-metalink.description": "如果设置为 是 或仅内存, 当后缀为 .meta4 或 .metalink 或内容类型为 application/metalink4+xml 或 application/metalink+xml 的文件下载完成时, aria2 将按 Metalink 文件读取并下载该文件中提到的文件. 如果设置为仅内存, 该 Metalink 文件将不会写入到磁盘中, 而仅会存储在内存中. 如果设置为 否  , 则 .metalink 文件会下载到磁盘中, 但不会按 Metalink 文件读取并且其中的文件不会进行下载.",
    ...
    "metalink-preferred-protocol.description": "指定首选使用的协议. 可以设置为 http, https, ftp 或无. 设置为无时禁用此选项.",
    ...
    "download-result.description": "此选项将修改下载结果的格式. 如果设置为 默认 , 将打印 GID, 状态, 平均下载速度和路径/URI. 如果涉及多个文件, 仅打印第一个请求文件的路径/URI, 其余的将被忽略. 如果设置为完整, 将打印 GID, 状态, 平均下载速度, 下载进度和路径/URI. 其中, 下载进度和路径/URI 将会每个文件打印一行. 如果设置为隐藏, 下载结果将会隐藏.",
    "dscp.name": "DSCP",
